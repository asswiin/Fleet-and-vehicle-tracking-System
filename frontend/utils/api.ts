...
export const api = {
  ...
  // MANAGER PROFILE IMAGE UPLOAD (Fixed Endpoint)
  updateUserProfileWithImage: async (id: string, formData: FormData) => {
    // Corrected Endpoint: /api/users/:id (standard PUT)
  ...
  // DRIVER IMAGE UPLOAD (Fixed Headers)
  updateDriverProfileWithImage: async (id: string, formData: FormData) => {
    const url = `${API_BASE_URL}/api/drivers/${id}`;
    try {
      const response = await fetch(url, {
  ...
  // NOTIFICATIONS
  createNotification: (data: any) => apiCall("/api/notifications", { method: "POST", body: JSON.stringify(data) }),
  ...
  reassignTrip: (tripId: string, data: { newDriverId: string; managerId: string }) => apiCall(`/api/trips/reassign/${tripId}`, { 
    method: "PATCH", 
    body: JSON.stringify(data) 
  }),
};
